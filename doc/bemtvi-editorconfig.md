<!-- DO NOT EDIT doc/bemtvi-editorconfig.txt BY HAND. It is generated from this file by
panvimdoc — run `scripts/gen-vimdoc.sh` after editing. -->

EditorConfig support for bemtvi, built entirely on the native `btv.*` plugin API — nothing about
...
read goes through the async `btv.fs` seam, so it never blocks the editor tick and works the same
...
<!-- Passed through verbatim so `:help bemtvi-editorconfig` lands on this page
...
`plugin/bemtvi-editorconfig.lua` auto-sources on load and registers everything with defaults, so
...
These EditorConfig properties map onto a real bemtvi option and are applied:
...
`trim_trailing_whitespace = true` runs on `BufWritePre`, which in bemtvi fires *before* the buffer
...
`insert_final_newline` needs no work to satisfy in the `true` direction: bemtvi's rope always keeps
one trailing newline, and `:w` writes the rope as-is, so a saved file always ends with exactly one.
Setting it to `false` asks for the opposite, which bemtvi cannot express (there is no
`'endofline'`-style option) — so instead of ignoring it, the plugin warns once per buffer.

`max_line_length` is resolved but not applied: bemtvi has no `'textwidth'`. Per the EditorConfig
...
if props and props.max_line_length then
  btv.wo.colorcolumn = props.max_line_length   -- 'colorcolumn' is window-local
end
```
...
A buffer opened by a relative path (`bemtvi src/main.rs`, `:e sub/file.txt`) is resolved against the
...
`btv.view` dock — is skipped on its `'buftype'`, so EditorConfig never sets `'expandtab'` on a
listing you cannot edit.

# Ordering

The resolution is async, but it is not a race. bemtvi's read lifecycle is a **gated chain** —
...
btv.on("FileType", {}, function(args)
  ...
  print(btv.bo[args.buf].shiftwidth)
end)
```
...
`require("bemtvi-editorconfig").setup(opts)`

Registers the autocmds that drive everything. `plugin/bemtvi-editorconfig.lua` calls it for you when
...
`require("bemtvi-editorconfig").properties(bufnr)`
...
`require("bemtvi-editorconfig").resolve(bufnr, file)`
...
Everything is public `btv.*`, and worth reading if you are writing a plugin:

- `btv.on` / `btv.augroup` — `BufReadPost` / `BufNewFile` drive the first resolution, `BufWinEnter`
...
- `btv.fs.read_text` inside `btv.async` / `btv.await`, fanned out with `btv.promise.all_settled` — every
...
- `btv.utils.ancestors` / `btv.fname.modify` — the upward directory walk and the relative→absolute
  resolution behind it.
- `btv.bo[bufnr]` — the buffer options the properties map onto, and `'buftype'` as the canonical
  "is this a real file" signal.
- `btv.buf.lines` / `btv.buf.set_lines` — the trim reads the buffer mirror, rejects a line in O(1)
  ...
  `btv.cmd([[%s/\s+$//]])` is 2–12x slower on the same buffer — it runs the regex engine over every
  line even when nothing matches — moves the cursor, clobbers the search register, and echoes
  `E486: Pattern not found` on every save of an already-clean file, since bemtvi's `:s` has no `e`
  flag. (`btv.cmd(cmd, { emsg_silent = true })` would silence that last one, and none of the rest.)

The parser (`bemtvi-editorconfig.parse`) turns one file into its `root` flag plus its sections in
file order. The glob dialect is the plugin's own (`bemtvi-editorconfig.glob`): brace expansion into
plain globs, then a backtracking matcher. That is deliberate, not an oversight — bemtvi ships one
glob engine in `btv.glob`, but EditorConfig's dialect differs from it in two ways that matter:
`**` crosses `/` **anywhere** (so `[**.js]` matches `a/b/c.js`, where `btv.glob` treats `**` as
recursive only when it is a whole path component), and `{1..9}` is a numeric range, which `btv.glob`
...
The help file is generated from `doc/bemtvi-editorconfig.md` by `scripts/gen-vimdoc.sh`; a pre-push
