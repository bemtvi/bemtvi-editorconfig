...
--
-- **Why not `btv.glob`?** bemtvi now ships one glob engine (`btv.glob`, globset compiled
...
--           `a**z` matches `a/mn/z`. btv.glob (shell/gitignore): `**` is recursive
--           only as a whole path component, so `**.js` degrades to `[^/]*[^/]*\.js`
--           and quietly stops matching nested files.
-- {m..n}    EditorConfig: a numeric range, `{1..3}` matching `1`/`2`/`3`. btv.glob has
--           no ranges — a comma-less `{…}` is one literal alternative.
-- ```
--
-- Translating instead of matching (`btv.glob.to_regex` onto `btv.regex`) would not help:
...
-- swap to `btv.glob` looks correct and silently breaks `[**.js]` sections.
