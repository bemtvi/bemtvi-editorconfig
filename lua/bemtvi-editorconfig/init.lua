-- bemtvi-editorconfig — `.editorconfig` support for bemtvi, built entirely on the
-- native `btv.*` plugin API.
--
...
-- filesystem access goes through the async `btv.fs` seam (`btv.async` + `btv.await`)
...
--
-- The read handler **returns** its promise, so bemtvi's gated read chain waits for the
...
--
-- Properties honored (the ones that map to a real bemtvi option — per the
...
-- ```
--
-- Plus the write-time pair, on `BufWritePre` (which in bemtvi fires *before* the
...
--
-- `max_line_length` is resolved but not applied — bemtvi has no `'textwidth'`. It, and
-- anything else in the file, is still exposed through
-- `require("bemtvi-editorconfig").properties(bufnr)` for a config to act on.

local glob = require("bemtvi-editorconfig.glob")
local parse = require("bemtvi-editorconfig.parse")

local M = {}

-- Resolved property table per bufnr, exposed via `M.properties` (and the source for
-- anything acting on properties with no bemtvi option).
M._resolved = {}
  ...
  for _, opt in ipairs(MANAGED) do
    ...
    if value ~= nil then
      if baseline[opt] == nil then
        baseline[opt] = btv.bo[bufnr][opt]
      end
      btv.bo[bufnr][opt] = value
      applied[opt] = true
    elseif previous[opt] and baseline[opt] ~= nil then
      btv.bo[bufnr][opt] = baseline[opt]
    end
  end
...
M.resolve = btv.async(function(bufnr, file)
  -- A buffer's name is the path as typed, so it is routinely *relative*
  -- (`bemtvi src/main.rs`, `:e sub/file.txt`). Resolve it against the editor's cwd up
  ...
  file = btv.fname.modify(file, ":p")
  M._files[bufnr] = file

  -- Every ancestor directory, nearest first.
  local dirs = {}
  for dir in btv.utils.ancestors(file) do
    dirs[#dirs + 1] = dir
  end
  ...
  for i, dir in ipairs(dirs) do
    reads[i] = btv.fs.read_text(dir .. "/.editorconfig")
  end
  local settled = btv.await(btv.promise.all_settled(reads))
...
-- which in bemtvi runs *before* the buffer is serialized, so the trimmed text is what
-- lands on disk.
--
-- Deliberately NOT `btv.cmd([[%s/\s+$//]])`, which is both slower and wrong here: the
...
-- clobbers the search register, and — since bemtvi's `:s` has no `e` flag — echoes
-- `E486: Pattern not found` on every save of an already-clean file. bemtvi now *does*
-- have the modifier (`btv.cmd(cmd, { emsg_silent = true })` runs it under `:silent!`),
...
local function trim_trailing(bufnr)
  if btv.bo[bufnr].modifiable == false then
    return
  end
  local lines = btv.buf.lines(bufnr, 0, -1, false)
  local first, last
  for i = 1, #lines do
  ...
  btv.buf.set_lines(bufnr, first - 1, last, false, slice)
end
...
-- `insert_final_newline` against bemtvi's text model. The rope *always* keeps a single
-- trailing newline (the final phantom line is never edited or shown) and `:w` writes
-- the rope as-is, so every file bemtvi saves already ends with one: `= true` is
...
local function check_final_newline(bufnr, props)
  ...
  btv.notify(
    "bemtvi-editorconfig: insert_final_newline = false is not supported — bemtvi always "
...
local function run(bufnr, file)
  return M.resolve(bufnr, file):catch(function(err)
    btv.notify("bemtvi-editorconfig: " .. tostring(err), vim.log.levels.WARN)
  end)
end
...
-- `plugin/bemtvi-editorconfig.lua` when the plugin is on the runtimepath, so a user
...
function M.setup(opts)
  ...
  local grp = btv.augroup.create("bemtvi-editorconfig")
  ...
  local function on_open(ev)
    ...
    if (btv.bo[bufnr].buftype or "") ~= "" then
      return
    end
    -- RETURNED, not fire-and-forget: bemtvi's read chain is gated on the promise a
  ...
  btv.on("BufReadPost", { group = grp }, on_open)
  btv.on("BufNewFile", { group = grp }, on_open)
  ...
  btv.on("BufWinEnter", { group = grp }, function(ev)
  ...
  btv.on("BufWritePre", { group = grp }, function(ev)
    local bufnr = ev.buf
  ...
  btv.on("BufWritePost", { group = grp, pattern = "**/.editorconfig" }, function(ev)
    local file = ev.file
    if type(file) ~= "string" or file == "" then
      return
    end
    local dir = btv.utils.dirname(btv.fname.modify(file, ":p")) .. "/"
    for bufnr, path in pairs(M._files) do
      ...
      if path:sub(1, #dir) == dir and btv.buf.is_loaded(bufnr) and enabled(bufnr) then
        run(bufnr, path)
      end
    end
  end)
  ...
  btv.on("BufDelete", { group = grp }, function(ev)
    local bufnr = ev.buf
...
-- bemtvi option (e.g. `trim_trailing_whitespace`, `max_line_length`) so a caller can
